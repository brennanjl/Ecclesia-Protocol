...
export var getComments = async (postTXID, numOfPosts) => {
  ...
  for (let comment in getID.data.transactions.edges) {
    if (
      typeof getID.data.transactions.edges[comment].node.tags[5].value !=
      "undefined"
    ) {
      ...
      commentList.push({
        comment: getID.data.transactions.edges[comment].node.tags[5].value,
        timeStamp: getID.data.transactions.edges[comment].node.tags[4].value,
      });
