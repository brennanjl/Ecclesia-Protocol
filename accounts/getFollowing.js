...
export var getFollowing = async(publicKey) => {
    ...
    const getFollowingList = ( await run(`query($cursor: String) {
...
for (let post in getFollowingList.data.transactions.edges) {
  postList.push({"timeStamp": getFollowingList.data.transactions.edges[post].node.tags[4].value, "TXID": getFollowingList.data.transactions.edges[post].node.id})
}
...
let followList = await getDataFromTXID(recentFollowList[0].TXID)
 ...
 //getFollowing('Cf1cXx1wENt0XOA9wMoTWYB-rvP0jEdGS1gdQN7XkvQ')